	...
	def alien_show(self):
		for i in range(21):
			...
			if self.x_ball > ui.bx[i] - self.size_ball and \
				...
				self.x_ball < ui.bx[i] + 53 and \
				self.y_ball < ui.by[i] + 25:
				# комета
				#ui.a[i].setDisabled(True)
				#ui.bx[i] = 0
				#ui.by[i] = 0
				if self.x_ball > ui.bx[i] - self.size_ball + 4 and self.x_ball < ui.bx[i] + 53 - 4:
					self.speed_y = -self.speed_y				
				if self.y_ball > ui.by[i] - self.size_ball + 4 and self.y_ball < ui.by[i] + 25 - 4:
